...
**Import contracts:** import-linter (`lint-imports`) in CI with the contracts from [plugin-architecture.md](plugin-architecture.md) and [feature-packages.md](feature-packages.md), written against the flat names via `root_packages`: a `layers` contract over the six layers, `forbidden` contracts (features and capabilities never import `infrastructure` or `server`; `contracts` imports nothing from the app; only `server` imports provider modules), and `independence` across the packages in `features/`. They land before any package moves; layers that don't exist yet are marked optional. The ratchet mechanism: existing violations are enumerated in per-contract `ignore_imports` with `unmatched_ignore_imports_alerting = error`, so the list only shrinks and stale entries are flagged. A deprecated-import guardrail (a freeze baseline that only ratchets down) plus a client-usage-matrix report guard the client-layer migration, and are retired once their baselines empty.
...
Tickets: toolchain convergence (Phase 2); import-linter contracts (TASK-18). Tolerated until closed: black formatting, soft-fail mypy, 3.11 CI / 3.12 venv lagging the 3.14 image, optional-dependencies dev group; no import-linter configuration yet.

**Changes:**
- 2026-09-24: import-linter contracts follow the six layers of plugin-architecture.md and land before any move.
