...
Tickets: TASK-26 (runtime and Web client homes), TASK-33 (async Bolt), TASK-41 (legacy-list removal, [migration.md](migration.md)), TASK-24 (settings). Tolerated until closed: mixed sync/async Bolt (an `AsyncApp` already exists in `bootstrap.py` while `provider.py` runs the sync `SocketModeHandler`), current dual registration, transport code (parser, formatter) still under `integrations/slack/`, and `infrastructure/configuration/infrastructure/platforms.py`, a third Slack settings home that is deleted, never extended.

**Changes:**
- 2026-09-24: dropped the closed `PREFIX` retirement (TASK-45); Migration names epic tickets only.
