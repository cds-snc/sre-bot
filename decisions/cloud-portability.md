...
4. **Every backing service is reached through a capability-shaped Protocol, and each such Protocol has an in-memory fake exercised by tests.** This is the driven-port ("secondary adapter") half of [ports-and-adapters](https://alistair.cockburn.us/hexagonal-architecture/): the in-memory adapter is the standing *second* adapter that proves the port is technology-neutral without building Azure, and it doubles as the fast test double every consumer's tests use. "Fake" is meant in the [Meszaros/Fowler sense](https://martinfowler.com/bliki/TestDouble.html) — a *working in-memory implementation the test seeds* (like an `InMemoryTestDatabase`), **not** production data baked into the repo, and preferred over `MagicMock` ([dependency-injection.md](dependency-injection.md)). A Protocol that can't be faithfully faked because it leaks vendor syntax fails this contract — `StorageService.query(key_condition: str)` is the counterexample and must be redesigned before any second backend.
   ...
   **Scope — and the anti-dead-code rule.** The fake contract applies only to **Path A ports that face an external backing service** (storage, queue, idempotency, directory as an IDP-neutral port, the approval-workflow store per [approvals.md](approvals.md)). It does **not** apply to in-process mechanisms that have no vendor to substitute: the event dispatcher, logging, and the plugin registry *are* their own implementation and are exercised directly in tests — a "fake dispatcher" would be pure dead code. And a fake is required only where the capability has a real consumer (Path A capabilities are never built speculatively, [layers.md](layers.md)). Where consumers exist their tests already hand-roll ad-hoc doubles — the Access Sync suite's inline `DirectoryProvider` stub, the audit `write_audit_event` monkeypatch — so the shared fake *removes* duplication rather than adding code.
...
- **CI-enforced:** every Path A backing-service package under `app/infrastructure/<service>/` contains a fake implementation exercised by tests (a CI check fails the build on a Path A Protocol with no fake); in-process mechanisms with no external vendor (events, logging, plugins) are out of scope. Current gaps: `directory/` (no shared `InMemoryDirectoryProvider` — tests hand-roll one) and `audit/` (rides `StorageService`, blocked on the `query()` redesign) — both closed by TASK-27.
...
Tickets: storage-protocol redesign + Path A fakes + the CI fake-coverage check (TASK-27); `QueueService` fake (TASK-34); pipeline validation switch. Tolerated until closed: single (AWS) implementation per Protocol; the `directory/` and `audit/` fake gaps named in Checks; CloudWatch-based deploy check.
