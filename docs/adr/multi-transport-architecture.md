...
- Imports raw vendor-SDK access exclusively from `app/integrations/<platform>/` per `client-module-placement.md`. The vendor-SDK package contains only authenticated client construction and primitive verification utilities; the infrastructure service composes them into the application's Protocol surface.
...
HTTP does not have a corresponding `app/infrastructure/http/` service because HTTP is not a vendor — it is the application's own protocol. HTTP outbound calls (when a feature calls a vendor over HTTP) go through that vendor's `app/integrations/<vendor>/` and corresponding infrastructure service.
...
- **Infrastructure-composition phase.** Each platform's infrastructure service is constructed: vendor clients (from `app/integrations/<platform>/`) wired against credentials from the settings; the service Protocol is exposed via its provider.
...
- **Static checks.** The import contract enforces that feature code does not import from `app/integrations/<platform>/` (vendor-import rule) and that platform-specific infrastructure services are reached only through their Protocols.
