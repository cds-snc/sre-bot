...
| `TRANSIENT_ERROR` | The call failed for reasons likely to clear on retry (rate limit, throttling, timeout, upstream 5xx, network blip). The result *may* carry `retry_after` indicating when retry is appropriate, when the upstream provided a hint. | Yes |
...
- `retry_after` — seconds until retry is appropriate. **Optional on `TRANSIENT_ERROR`**; populate only when the upstream provided a hint (HTTP `Retry-After` header, SDK rate-limit attribute, equivalent). Absent on all other statuses and absent on `TRANSIENT_ERROR` when no upstream hint exists — consumers must handle the absent case by applying their own backoff (queue redelivery cadence, edge-mapping default, etc.).
...
- Inside vendor client modules under `app/integrations/` (which raise typed SDK exceptions — see [client-adapter-responsibilities.md](client-adapter-responsibilities.md) and [client-module-placement.md](client-module-placement.md)).
...
`retry_after` on `TRANSIENT_ERROR` is **information**, not invitation to loop. The client layer's job is to surface the hint from the SDK's final state, not to act on it by wrapping the SDK call in a project-local retry loop. SDK-native retry primitives — boto3 `Config(retries={"mode": "standard"})`, `slack_sdk` retry handlers, `googleapiclient.HttpRequest.execute(num_retries=...)`, kiota `RetryHandlerOption` — execute *inside* the awaitable and complete before the client's classifier sees the outcome; what the client wraps in `OperationResult` is the post-retry final state, not an intermediate attempt. See [outbound-retry-policy.md](outbound-retry-policy.md) Shape A for the binding rule on how SDK retry is configured at construction time, and [client-adapter-responsibilities.md](client-adapter-responsibilities.md) for the split between resilience (construction) and classification (call-time).

The named consumers of `retry_after` are layers *above* the client boundary:

- **Queue redelivery** ([message-queuing.md](message-queuing.md)) — when a consumer returns `TRANSIENT_ERROR` and the message is left to lapse, the consumer runner may use `retry_after` to extend the visibility timeout so the broker waits at least that long before redelivery, instead of redelivering on its default cadence (and burning a delivery attempt against an upstream that explicitly asked for more time).
- **HTTP edge** ([api-design-error-mapping.md](api-design-error-mapping.md)) — maps `OperationResult.retry_after` to the RFC 9457 / HTTP `Retry-After` response header so upstream callers back off correctly.
- **Platform rendering** (Slack, Teams) — surfaces "try again in N seconds" instead of a generic "try later" message when the value is available.
- **Handler-driven scheduling** ([handler-idempotency.md](handler-idempotency.md)) — a handler that wants to schedule a retry after an explicit delay (rather than rely on queue redelivery) uses `retry_after` as the delay.

When `retry_after` is absent on a `TRANSIENT_ERROR`, each consumer falls back to its own default. The absence is normal — connection resets, TLS timeouts, and generic 5xx responses without a `Retry-After` header all yield `TRANSIENT_ERROR` without a meaningful hint. Forcing the classifier to invent a value would fabricate information.
...
- **Code review (clients).** Vendor client modules under `app/integrations/` do not import `OperationResult` or any status value.
- **Static analysis.** The status enumeration is typed as a closed `Literal` so omitting a status produces a type-checker error. Import-linter (or equivalent) forbids `OperationResult` imports inside `app/integrations/`.
...
- 2026-05-12: **Clarification (no normative change).** The envelope's primary justification is layered-architecture isolation — feature code does not see vendor exception types (`SlackApiError`, `ClientError`, `HttpError`, `ODataError`). Its role as a retry-information carrier (the `retry_after` field on `TRANSIENT_ERROR`) is secondary. An SDK-capability audit ([../../tmp/research-shield-pattern-sdk-capabilities.md](../../tmp/research-shield-pattern-sdk-capabilities.md)) found that the existing client-layer executors had read the envelope's existence as licensing a hand-rolled retry loop in our code above the SDK call. That reading is wrong: the envelope does not retry, does not coordinate retries, and is not the place where retries are decided. The client's resilience responsibility is met by configuring SDK-native retry primitives at construction time (see [outbound-retry-policy.md](outbound-retry-policy.md) Shape A and [client-adapter-responsibilities.md](client-adapter-responsibilities.md) revised 2026-05-12); the envelope carries the *final* outcome the SDK produced after its retries ran. The Retry semantics subsection has been expanded to spell out that `retry_after` is information, not invitation to loop. The status set, shape, boundary scope, and consumer-decides-on-retry contract are unchanged.
- 2026-05-12: **Relaxed `retry_after` from mandatory to optional on `TRANSIENT_ERROR`.** Background: the original "Mandatory on `TRANSIENT_ERROR`" wording was paired with the (since-removed) implication that the client layer would loop on the hint. With looping moved to SDK-native retry inside the awaitable, the field's role is purely informational for outer layers (queue redelivery visibility-timeout extension, HTTP edge `Retry-After` mapping, platform "try again in N seconds" rendering). Many transient outcomes legitimately have no upstream hint — connection resets, TLS timeouts, generic 5xx without `Retry-After`, raw network errors — and forcing the classifier to invent a value would fabricate information that outer consumers might then act on. The relaxed contract: populate `retry_after` only when the upstream provided a hint (HTTP `Retry-After` header, SDK rate-limit attribute, equivalent); absent on `TRANSIENT_ERROR` is a valid state that each outer consumer handles by applying its own default. The named consumers of `retry_after` are now enumerated in the Retry semantics subsection so the field's purpose is unambiguous. The status set, shape (other fields), boundary scope, retry semantics (`OperationResult` does not retry), and consumer-decides-on-retry contract are unchanged.
