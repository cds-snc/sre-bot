...
- **Repository contents.** A single ASGI middleware (under `app/server/` or equivalent) handles inbound correlation. A single HTTP-client wrapper (under `app/integrations/_http/` or equivalent) propagates outbound headers. The structured-logging configuration registers `structlog.contextvars.merge_contextvars` as a processor.
