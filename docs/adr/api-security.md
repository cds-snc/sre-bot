...
- **Static analysis.** Bandit (per `code-quality-tooling.md`) catches common security anti-patterns; the import contract (per `import-governance.md`) catches feature code reaching into `app/integrations/` outside the adapter boundary. Neither is a substitute for code review on the security subsystem.
