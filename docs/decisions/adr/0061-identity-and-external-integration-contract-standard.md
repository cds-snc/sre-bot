...
| Slack command/interaction | Slack `user_id` from Bolt event payload (trusted via authenticated Socket Mode WebSocket connection — `SLACK_APP_TOKEN`; individual events are not HMAC-signed per ADR-0067 S2, S5-R2) | `SlackPlatformProvider` resolves via Slack `users.info` API | Platform layer (Category C per ADR-0078, ADR-0067 Standard 8) |
