...
> **Note (2026-04-29 - Platform Services Assessment):** The `PlatformService` facade and its convenience accessors will be retired when per-platform services (`SlackService`, `TeamsService`) are constructed directly via settings-gated lifespan wiring (ADR-0078). Three-file DI ceremony for the former `PlatformService` facade is no longer applicable; per-platform services may use simpler provider patterns since they are infrastructure-internal (Category C per ADR-0077).
