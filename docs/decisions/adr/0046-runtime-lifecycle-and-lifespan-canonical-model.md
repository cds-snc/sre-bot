...
- **Security service degraded-start (ADR-0064 S5 R5):** Missing JWT issuer configuration does not terminate startup. The application starts in degraded mode where authenticated endpoints return 401. This exception is scoped to security configuration only and is documented with explicit rationale in ADR-0064.
