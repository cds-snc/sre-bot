...
- Add `Library Adoption Decision` as a valid Tier-5 decision_type alongside `Migration Decision` and `Deprecation Decision` (ADR-0044 Tier-5 library trigger, ADR-0045 P7).
...
- Validation summary: Classification constraints match ADR-0044 governance baseline and Step 5 taxonomy reset objectives. Amended 2026-04-30 to add `Library Adoption Decision` as Tier-5 decision_type per ADR-0044 library trigger amendment.
- Follow-up actions:
...
- 2026-04-30: Library Adoption Decision amendment. Added `Library Adoption Decision` as a valid Tier-5 decision_type alongside `Migration Decision` and `Deprecation Decision`. Infrastructure library selections (Tier 2 in the delegation hierarchy per ADR-0045 P7) require a formal Tier-5 ADR documenting evaluation criteria (maturity, maintenance, licensing, type-hint coverage, async compatibility). Complements ADR-0044 Tier-5 library trigger amendment. See managed-services-delegation-adr-review-tracker-2026-04-30.md Item #10.
