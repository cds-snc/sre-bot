...
### Standard 9: Singleton Lock Lifecycle and Operator Intervention

Singleton lock lifecycle — acquisition, TTL expiration, release, and operator intervention — is a **platform concern owned by infrastructure**, not a feature-level responsibility. Lock release is the logical inverse of lock acquisition; both must be provided by the same infrastructure utility (Standard 4: "The singleton lock implementation must be provided as an infrastructure utility").

This standard aligns with Azure Well-Architected guidance: "Enforce single-instance execution when required" and "Alert on missed schedules" are platform-level scaling and observability responsibilities, not feature-local concerns.

**Lock lifecycle states:**

| State | Meaning | Transition |
|-------|---------|------------|
| `free` | No lock held; job eligible to run | → `held` (on successful conditional write) |
| `held` | Lock acquired by a running job instance | → `free` (on normal release or TTL expiration) |
| `stuck` | Lock held beyond expected job duration; owning process likely dead | → `free` (on TTL expiration, operator release, or stale-threshold override) |

**Rules:**

- **R1 (Infrastructure ownership):** Lock release operations — both programmatic (post-job) and manual (operator intervention) — must be implemented in the infrastructure layer (`app/infrastructure/`). Feature packages must not implement their own lock release utilities. Feature packages consume the lock infrastructure via Protocol contracts or decorated job functions.
- **R2 (TTL as primary recovery):** TTL-based automatic expiration (ADR-0057 Standard 4) is the primary recovery mechanism for stuck locks. The lock TTL must be calibrated to be longer than the expected maximum job duration but short enough to provide acceptable recovery time. Feature packages may configure their lock TTL via settings (ADR-0055), but the expiration mechanism is infrastructure-owned.
- **R3 (Stuck-lock observability):** The infrastructure lock utility must emit structured log events for lock state transitions:
  - `singleton_lock_acquired`: `job_name`, `lock_key`, `ttl_seconds`, `task_id`
  - `singleton_lock_released`: `job_name`, `lock_key`, `held_seconds`, `task_id`
  - `singleton_lock_ttl_expired`: `job_name`, `lock_key`, `ttl_seconds` (emitted when a lock is found expired during acquisition check)
  - `singleton_lock_skipped`: `job_name`, `lock_key`, `held_by_task_id` (emitted when a job skips due to active lock)
- **R4 (Operator release utility):** The infrastructure layer must provide a lock release utility that allows authorized operators to forcibly release a stuck singleton lock. This utility:
  - Operates on the lock infrastructure directly (DynamoDB conditional delete or TTL override), not through feature-specific application code.
  - Requires structured audit metadata: `operator_id`, `reason`, `lock_key`, `timestamp`.
  - Emits a structured event: `singleton_lock_operator_released` with full audit context.
  - Is idempotent: releasing an already-free lock is a no-op (log and return success).
  - May be exposed through multiple ingress paths (CLI script, HTTP admin endpoint, Slack command) — the ingress adapter is a consumer of the infrastructure utility, not a reimplementation.
- **R5 (Feature-scoped admission, not feature-scoped release):** Feature packages own job admission logic: deciding whether a job should attempt to acquire a lock based on domain preconditions (e.g., feature enabled, platform configured, sync not paused). Feature packages do NOT own lock release logic. The boundary is: "Should this job try to acquire the lock?" is feature-scoped; "Release this lock" is infrastructure-scoped.
- **R6 (No feature-level admin packages for lock management):** Feature packages must not create dedicated admin subpackages whose primary purpose is lock state management. If a feature needs operator visibility into its job state, it consumes the infrastructure lock observability events (R3) and the operator release utility (R4) — it does not reimplement these capabilities.

**Rationale:**

- Standard 4 already establishes that the lock *implementation* is infrastructure-owned. Logically, the lock *lifecycle* (including release and intervention) must be owned by the same layer.
- Feature-scoped lock release creates architectural debt: each feature reimplements authorization, audit, and DynamoDB operations that are identical across all singleton jobs.
- Platform-level operator intervention scales naturally: one utility covers all Tier 2 jobs across all features without per-feature admin packages.
...
1. Async background tasks via `asyncio.create_task()` on the FastAPI event loop:
...
1. Distributed job scheduling (e.g., Celery, AWS EventBridge + Lambda):
...
1. APScheduler as prescribed by legacy ADR-0015:
...
- Platform-owned lock lifecycle (Standard 9) prevents feature-scoped reimplementation of lock management and provides a single operator intervention path for all Tier 2 jobs.
- Tradeoffs accepted:
...
- Package/infrastructure boundary impact: Standard 3 defines the contract between feature packages (which register jobs) and infrastructure (which provides the scheduler and registry). Feature packages depend on the `BackgroundJobRegistry` Protocol, not on the scheduling library. Standard 9 reinforces this boundary for lock lifecycle: lock release (including operator intervention) is infrastructure-owned; feature packages consume it via Protocol or decorated functions.
- Type boundary impact: `BackgroundJobRegistry` is a `Protocol` (correct per ADR-0045 Principle 6 and ADR-0077 - behavior contracts use Protocol). The registry adapter is an infrastructure implementation detail. The lock release utility exposes a `SingletonLockManager` Protocol (or equivalent) for operator intervention consumers.
...
- Settings partitioning impact: Job schedules are currently hardcoded in the initialization module. If job schedules become configurable, they must follow ADR-0055 (feature-owned settings for feature jobs, infrastructure settings for infrastructure jobs). Lock TTL values are infrastructure settings (infrastructure owns the lock mechanism); feature packages may influence TTL via their job registration metadata.
- Operator intervention boundary impact: Standard 9 R6 explicitly prohibits feature packages from creating admin subpackages for lock management. Operator-facing ingress (HTTP admin endpoint, Slack command) for lock release is a thin adapter consuming the infrastructure utility — not a feature-level reimplementation.
...
11. Source title: Microsoft Azure Architecture - Best Practices for Background Jobs (Observability and Operator Intervention)

- URL: <https://learn.microsoft.com/en-us/azure/architecture/best-practices/background-jobs>
- Publisher/maintainer: Microsoft
- Accessed date (YYYY-MM-DD): 2026-04-30
- Relevance summary: "Alert on missed schedules" and "Enforce single-instance execution when required" confirm operator intervention and lock state monitoring as platform-level responsibilities, not feature-scoped. "Monitor dead-letter queues" and "Track job completion, not only job start" validate Standard 9 R3 observability requirements.

12. Source title: ADR-0057 Standard 4 (Crash Resilience)

- URL: docs/decisions/adr/0057-runtime-disposability-and-graceful-shutdown-standard.md
- Publisher/maintainer: SRE Team
- Accessed date (YYYY-MM-DD): 2026-04-30
- Relevance summary: "Long-held locks must have TTL-based expiration so that a killed process's locks are automatically released." Establishes TTL as the primary recovery mechanism; Standard 9 R2 codifies this as the precedence rule for operator intervention.
...
- 2026-04-30: Added Standard 9 (Singleton Lock Lifecycle and Operator Intervention). Establishes lock release as infrastructure-owned, prohibits feature-scoped admin packages for lock management, codifies operator intervention as a platform utility consumed by ingress adapters. Motivated by ADR-0043 rejection analysis: feature-scoped lock release contradicts Standard 4's infrastructure utility mandate. Updated Compliance and Boundaries, Consequences, Source References (§11-§12), metadata.
- 2026-04-30: Removed ADR-0066 from impacts metadata — ADR-0066 narrowed to config naming only (no lock scope content). ADR-0043 rejection stands independently via its own `status: Rejected` and this record's Standard 9.
