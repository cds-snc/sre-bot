...
Slice 2 (contract) of TASK-25.2.3, split 2026-09-11 under the single-PR size gate. Depends on TASK-25.2.3.1 (the adapter). Moves every live production caller of integrations.aws.identity_store onto packages/aws_platform/adapters/identity_center.build_identity_center_adapter(), deletes the dead AWS account access-request flow instead of fixing the defects the TASK-25.2.1 characterization pass pinned in it, deletes the legacy module and its 890-line test file, and prunes the two guard baselines.

Callers (re-grepped 2026-09-11; eight files, ten call sites): modules/aws/identity_center.py (list_users at :70/:79; create_user/delete_user/create_group_membership/delete_group_membership passed as callables to entities.provision_entities at :155/:171/:261/:282/:329/:348), modules/aws/aws.py (:158 inside request_aws_account_access, dead), modules/aws/aws_access_requests.py (:194 get_user_id, dead), modules/aws/ops_group_assignment.py (:20 get_group_id), modules/provisioning/users.py (:58 list_users), modules/provisioning/groups.py (:142 list_groups_with_memberships), jobs/revoke_aws_sso_access.py (:30 get_user_id, dead), jobs/scheduled_tasks.py (:128 healthcheck).

Re-scope 2026-09-14 (human decision): the AWS account access-request flow -- modules/aws/aws_access_requests.py, the /aws access command and aws_access_view registration in modules/aws/aws.py, and jobs/revoke_aws_sso_access.py -- is not operational in production. The help text already labels /aws access "currently disabled", and the revoke job is scheduled nowhere and only processes rows that flow writes. packages/access will re-provide the capability. The flow is deleted under TASK-25.2.3.2.4 and is NOT migrated: no code parity, no error-path fixes, no retargeted tests. Deleting a Slack command inside a frozen module is justified as a bug-fix-shaped change under decisions/migration.md rule 1 (dead surface), recorded here; no ADR amendment.

Human decisions 2026-09-11 on the error paths that still apply: bulk listings that fail raise the module-local DirectoryUsersUnavailableError / DirectoryGroupsUnavailableError (mirroring the Google branches) instead of crashing with TypeError; ops_group_assignment keeps its existing 'not found' failed status on NOT_FOUND and returns a failed status carrying the result message on any other non-success; request_aws_account_access is deleted with its pinned tests; the healthcheck registration becomes lambda: build_identity_center_adapter().healthcheck().is_success. entities.provision_entities is not changed: it tests `if response:` and an OperationResult instance is always truthy, so modules/aws/identity_center.py wraps each adapter write in a small local bridge that maps the entity dict to adapter arguments and returns result.data on success or False (logged with status and error_code) on failure. The 2026-09-11 access_view_handler and revoke-job error-path decisions are superseded by the 2026-09-14 deletion.

Test files to retarget (patch build_identity_center_adapter / a fake adapter instead of the legacy module): tests/unit/modules/aws/{test_identity_center_handler,test_aws_command_handler,test_ops_group_assignment_handler}.py, tests/unit/modules/provisioning/test_provisioning_users.py, tests/modules/provisioning/test_provisioning_groups.py, tests/integration/jobs/test_scheduled_tasks_integration.py. Test files deleted with the dead flow: tests/unit/modules/aws/test_aws_access_requests_handler.py, tests/unit/jobs/test_revoke_aws_sso_access.py, tests/integration/jobs/test_revoke_aws_sso_access_integration.py, the /aws access case in test_aws_command_handler.py, and the aws_access_requests case in tests/unit/integrations/aws/test_dynamodb_local_endpoint.py. tests/integration/integrations/aws/test_identity_store_conformance.py is NOT orphaned: it exercises moto against packages/access's adapter and never imports the legacy module, so it stays untouched. packages/access is not reworked.
...
---
2026-09-14 re-scope (human decision): the AWS account access-request flow (modules/aws/aws_access_requests.py, /aws access, jobs/revoke_aws_sso_access.py) is dead in production and will be replaced by packages/access. It is deleted, not migrated, and no code parity is kept. Assignment of work across subtasks: TASK-25.2.3.2.3 shrinks to ops_group_assignment.py only. TASK-25.2.3.2.4 takes the whole dead-flow deletion, plus removing the local-only aws_access_requests table from .devcontainer/dynamodb-create.sh, bin/seed.sh and bin/db.sh. That knowingly exceeds the ~400 production LOC size gate: the human chose to keep all dead-code deletion in one PR. The Terraform table, backup-selection and IAM references go to a separate follow-up task, so data retention can be checked before the prod table is destroyed. The coordinator plan's execution-order text still names the old .3 scope; this comment supersedes it.
---
<!-- COMMENTS:END -->
