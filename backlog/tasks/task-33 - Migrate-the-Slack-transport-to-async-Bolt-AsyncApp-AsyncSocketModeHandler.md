...
Steps:
1. In app/server/slack/ (after TASK-26.2 moves the runtime there): construct AsyncApp + AsyncSocketModeHandler on the app event loop; start in the lifespan transport phase, close at shutdown; delete the thread-based runner and the duplicate sync/async bootstrap.
...
Build AsyncApp and AsyncSocketModeHandler in app/server/slack/ (after TASK-26 under the new split), not in app/infrastructure/slack/. Re-scope when the Draft records are accepted (TASK-83.1).
---
