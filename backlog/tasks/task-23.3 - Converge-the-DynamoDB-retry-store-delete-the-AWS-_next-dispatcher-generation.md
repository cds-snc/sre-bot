...
9. `get_stats()` `:416`/`:429` — two direct `self._dynamodb.query(..., Select="COUNT")` calls, each `response.get("Count", 0)`. Each wrapped in try/except -> classify + `dynamodb_get_stats_failed` log + count stays `0` (today's degraded-return contract). `claimed_records` stays hardcoded `0`. Counts become page-scoped rather than aggregated — see D1; add one short comment line noting counts are single-query (not paginated).
10. `get_dlq_entries()` `:467` — like step 4: direct `query(..., Limit=limit)`, `response.get("Items", [])`, classified failure -> `dynamodb_get_dlq_entries_failed` + `return []`.
...
12. `infrastructure/audit/service.py:4` — docstring says "no direct boto3 or dynamodb_next calls"; reword to reference `StorageService` only (the named module ceases to exist). Docstring-only, no behavior.
...
15. `bin/baselines/sdk_typing_antipatterns.txt` — remove the three `_next` lines. Keep the header comments and every other entry.
...
17. `tests/unit/infrastructure/resilience/retry/conftest.py` — replace `mock_dynamodb_next` with `mock_dynamodb_client` (`MagicMock(spec=["put_item", "get_item", "delete_item", "update_item", "query"])`) whose defaults are real boto3 response shapes: `put_item/update_item/delete_item -> {}`, `get_item -> {"Item": {}}`, `query -> {"Items": [], "Count": 0}`. `dynamodb_retry_store` injects it into the constructor instead of monkeypatching a module, and keeps attaching it for test access (rename `_mock_dynamodb_next` -> `_mock_dynamodb`). Drop the now-unused `OperationResult` import.
18. `tests/unit/infrastructure/resilience/retry/test_dynamodb_store.py` — mechanical rewrite of the doubles only: success cases return plain dicts; failure cases use `side_effect=ClientError({"Error": {"Code": ...}}, "UpdateItem")`. Every existing assertion is preserved, with `table_name=` kwargs becoming `TableName=`. Add the AC#3 cases: `fetch_due` and `get_dlq_entries` with a non-empty `{"Items": [...]}` response (the shape that used to blow up), and `get_stats` with `{"Count": 5}`. Add `claim_record` non-conditional mapped failure (e.g. `AccessDeniedException`) -> `False` + error log, and one unmapped-code case asserting the raw `ClientError` propagates (R1). Update the module docstring (behavior-only wording, no module names that will no longer exist).
...
- AC#3 (real-shape fakes for `fetch_due`/`get_stats`/`get_dlq_entries`) -> steps 4, 9, 10, 17, 18 -> the non-empty `{"Items": [...]}` and `{"Count": 5}` cases in `test_dynamodb_store.py`.
...
- D1 (needs a reviewer call): `get_stats` currently intends to aggregate counts across pages but in fact always returns 0 (see Grounding). The plan replaces it with a single `query(..., Select="COUNT")` per status, so counts become page-scoped (one query, up to 1 MB scanned). The alternative is `self._dynamodb.get_paginator("query")` summing `page["Count"]`, which restores the aggregate intent at the cost of a paginator fake in the tests. Recommendation: single call — the method is already documented as approximate (`claimed_records` is hardcoded 0) and has zero callers. Confirm or ask for the paginator variant.
...
- R1 (the one intentional behavior change): an UNMAPPED SDK error now propagates as the raw botocore exception instead of becoming `OperationResult.permanent_error`. Concretely `claim_record`/`fetch_due`/`get_stats`/`get_dlq_entries` can now raise where they previously returned `False`/`[]`/zeros. Mapped codes (including `ConditionalCheckFailedException`, `AccessDeniedException`, throttling, `ResourceNotFoundException`) keep today's outcomes, which is what AC#2 requires. Required by decisions/outbound-clients.md; the only caller path is `retry/worker.py:113,141`, which is not wired in production.
