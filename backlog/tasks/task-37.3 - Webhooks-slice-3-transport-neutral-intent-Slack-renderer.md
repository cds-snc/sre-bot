...
Slice 3 of the webhooks rearchitecture (decisions/webhooks.md; coordinator TASK-37). Removes Slack coupling from the capability core and deletes the filesystem-walk pattern registries.
...
2. dispatch.py: a Slack render target mapping intent -> Slack message via the Slack transport's OperationResult->message renderer / SlackService Protocol (decisions/platform-transports.md, decisions/transport-slack.md). The capability core (schemas/service/router/domain) imports NO slack_sdk and builds NO Block Kit. If infrastructure/slack/ (the platform transport) is not yet available, use a thin local SlackPoster Protocol backed by the existing bot client via a provider, with a comment pointing at platform-transports.md - do NOT import WebClient into the capability core.
...
Rendering goes through the outbound messaging Protocol of the Slack handler contract (TASK-26.1); no interim SlackPoster shim.
<!-- SECTION:DESCRIPTION:END -->

## Acceptance Criteria
<!-- AC:BEGIN -->
- [ ] #1 The webhooks capability core imports no slack_sdk/WebClient and constructs no Block Kit; Slack shapes exist only behind the transport renderer, which replies through the Slack handler contract's outbound messaging Protocol (grep + review)
