...
class AWSHelpers:
    """High-level AWS helper operations factory.

    Provides convenience methods for complex AWS operations that compose
    multiple low-level AWSClientFactory calls (batch operations, orchestration).

    Example:
        @router.get("/groups/{store_id}/with-members")
        def list_groups(aws_helpers: AWSHelpersDep, store_id: str):
            result = aws_helpers.list_groups_with_memberships(store_id)
            if result.is_success:
                return result.data
    """

    def __init__(self, aws: AWSClientFactory):
        self._aws = aws
    ...
    def list_groups_with_memberships(
        ...
        # 1. Fetch all groups
        groups_result = self._aws.list_users(
        ...
        users_result = self._aws.list_users(
            identity_store_id=identity_store_id,
            role_arn=role_arn,
            **(users_kwargs or {}),
        )
    ...
    def healthcheck(self, identity_store_id: str) -> bool:
        ...
        try:
            ...
            result = self._aws.list_users(identity_store_id=identity_store_id)
    ...
    def _fetch_group_memberships_parallel(
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            future_to_gid = {
                executor.submit(
                    self._aws.list_users,  # Using list_users as a placeholder for group memberships
                    identity_store_id=identity_store_id,
                    role_arn=role_arn,
