...
def _is_test_environment() -> bool:
    ...
    Returns:
        bool: True if pytest is in sys.modules, False otherwise.
    """
...
def get_logger(name: Optional[str] = None) -> BoundLogger:
    ...
    Returns:
        BoundLogger: Configured logger instance with context.
    """
    if name:
...
def get_module_logger() -> BoundLogger:
    """Get a logger for the calling module (backward compatibility).

    This is an alias for get_logger() with no arguments, provided for
    backward compatibility with code that uses get_module_logger().

    Returns:
        BoundLogger: Configured logger instance with module context.
    """
    # Auto-detect calling module
    current_frame = inspect.currentframe()
    if current_frame is None:
