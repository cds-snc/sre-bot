...
@patch("jobs.revoke_aws_sso_access.aws_sso")
@patch("jobs.revoke_aws_sso_access.aws_access_requests")
def test_revoke_aws_sso_access(
    aws_access_requests_mock, aws_sso_mock, log_ops_message_mock
):
    client = MagicMock()
    ...
    aws_sso_mock.get_user_id.return_value = "test_aws_user_id"
    aws_sso_mock.remove_permissions_for_user.return_value = True
    ...
    assert aws_sso_mock.get_user_id.call_count == 1
    assert aws_sso_mock.get_user_id.call_args == call("test_email")
    assert aws_sso_mock.remove_permissions_for_user.call_count == 1
    assert aws_sso_mock.remove_permissions_for_user.call_args == call(
...
def test_revoke_aws_sso_access_with_exception(
    logging_mock, aws_sso_mock, aws_access_requests_mock
):
    client = MagicMock()
    aws_access_requests_mock.get_expired_requests.return_value = [
        {
    ...
    aws_sso_mock.get_user_id.side_effect = Exception("test_exception")
    revoke_aws_sso_access.revoke_aws_sso_access(client)
