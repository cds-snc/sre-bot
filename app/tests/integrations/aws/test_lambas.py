...
from integrations.aws.lambdas import get_layer_version, list_functions, list_layers
...
def test_list_functions(mock_execute_aws_api_call):
    ...
    mock_execute_aws_api_call.assert_called_once_with("lambda", "list_functions", paginated=True, keys=["Functions"])
    assert result == [
        {"FunctionName": "function1"},
        {"FunctionName": "function2"},
    ]
...
def test_list_layers(mock_execute_aws_api_call):
    ...
    result = list_layers()
    mock_execute_aws_api_call.assert_called_once_with("lambda", "list_layers", paginated=True, keys=["Layers"])
    assert result == [
        {"LayerName": "layer1", "LatestMatchingVersion": {"Version": 1}},
        {"LayerName": "layer2", "LatestMatchingVersion": {"Version": 23}},
    ]
...
def test_get_layer_version(mock_execute_aws_api_call):
    ...
    mock_execute_aws_api_call.assert_called_once_with("lambda", "get_layer_version", LayerName="layer1", VersionNumber=1)
