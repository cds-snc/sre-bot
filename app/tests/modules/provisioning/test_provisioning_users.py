...
@patch("modules.provisioning.users.google_directory")
def test_get_users_from_integration_filters_applied(mock_google_directory, mock_filters):
    users_list = ["user1", "user2"]
    mock_google_directory.list_users.return_value = users_list
    mock_filters.filter_by_condition.return_value = ["user1"]
    users_list_filtered = users.get_users_from_integration("google_directory", processing_filters=["filter1"])
