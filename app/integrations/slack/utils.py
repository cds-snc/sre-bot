...
def generate_slack_idempotency_key(request_body: dict) -> str:
    ...
    if "actions" in request_body and isinstance(request_body["actions"], list) and request_body["actions"]:
    ...
    if "view" in request_body:
        return f"slack:view:{user_id}:{request_body['view'].get('callback_id', 'unknown')}"
...
def legacy_slack_listener(sync_func):
    ...
    async def async_wrapper(ack, body, *args, **kwargs):
        ...
        if cached_response:
            logger.info(f"Duplicate Slack event intercepted for key: {cache_key}. Dropping.")
            await ack()
            return
        ...
        await asyncio.to_thread(idempotency.set, cache_key, {"status": "in_flight"}, ttl_seconds=30)
        ...
        try:
            ...
            await asyncio.to_thread(idempotency.set, cache_key, {"status": "processed"}, ttl_seconds=3600)
            return result
        except Exception as e:
            logger.error(f"Execution failed. Evicting lock: {cache_key}")
            await asyncio.to_thread(idempotency.set, cache_key, {"status": "failed"}, ttl_seconds=5)
            raise e
