...
def list_functions():
    ...
    response = execute_aws_api_call("lambda", "list_functions", paginated=True, keys=["Functions"])
...
def list_layers():
    ...
    response = execute_aws_api_call("lambda", "list_layers", paginated=True, keys=["Layers"])
...
def get_layer_version(layer_name, version_number):
    ...
    """
    log = logger.bind(operation="get_layer_version", layer_name=layer_name, version=version_number)
